...
async function getKafkaData() {
    try {
        ...
        SELECT 
            ...
            CASE WHEN c.Cycle = 6 THEN 0 ELSE c.Cycle END AS cycle, -- Adjust cycle value
...
async function updateCycleStatus() {
    try {
        const updateCycleStatusQuery = `
            ...
            SET f.cycle_status = 'OFF' , t.end_time = CURRENT_TIME()
            WHERE i.Status = 'irrigated' AND f.cycle_status = 'ON' AND t.end_date = CURDATE();
        `;
